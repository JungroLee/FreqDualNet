...
class FrequencyChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_weight = attn_weight.view(B, C, 1, 1, 1)           # (B, C, 1, 1, 1)
...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.lower_thresh_bg = nn.Parameter(torch.tensor(0.7))
        self.upper_thresh_bg = nn.Parameter(torch.tensor(1.3))
        self.upper_thresh_org = nn.Parameter(torch.tensor(0.3))
        self.enc1_down = UpSampleBlock(12, 12, scale_factor=2)
        self.enc3_up = UpSampleBlock(48, 12, scale_factor=8)

        self.tumor_head = UnetOutBlock(spatial_dims=spatial_dims, in_channels=feature_size * 2, out_channels=2)
        self.organ_head = UnetOutBlock(spatial_dims=spatial_dims, in_channels=feature_size * 2, out_channels=2)
        self.freq_attn1 = FrequencyChannelAttention(feature_size)
        self.freq_attn2 = FrequencyChannelAttention(feature_size)
        self.freq_attn3 = FrequencyChannelAttention(2 * feature_size)
        self.freq_attn4 = FrequencyChannelAttention(4 * feature_size)
    ...
    def fft_features(x):
        ...
        returns: magnitude spectrum of shape (B, C, H, W, D)
        """
        # Apply FFT across last 3 dimensions: H, W, D
...
# 어느정도 거리 범위를 넘어가는 것만 통과하기
    def high_pass_filter(self, x, radius_ratio=0.4):
        """
        x: Tensor of shape (B, C, H, W, D)
        radius_ratio: how much low-freq region to suppress (0~0.5)
        returns: high-passed feature map (real part only)
        """
        x = x.float()
        B, C, H, W, D = x.shape
        ...
        dist = torch.sqrt(xx**2 + yy**2 + zz**2)
        mask = (dist >= radius_ratio).float()  # high-pass mask

        # Apply mask
        mask = mask.unsqueeze(0).unsqueeze(0)  # shape (1,1,H,W,D)
        fft_filtered = fft * mask

        # IFFT
        fft_filtered = torch.fft.ifftshift(fft_filtered, dim=(-3, -2, -1))
        filtered = torch.fft.ifftn(fft_filtered, dim=(-3, -2, -1))
        
        return filtered.real  # discard imaginary part
    def low_pass_filter(self, x, radius_ratio=0.25):
        """
        x: Tensor of shape (B, C, H, W, D)
        radius_ratio: how much high-frequency to remove (0~0.5)
        returns: low-passed feature map (real part only)
        """
        x = x.float()
        B, C, H, W, D = x.shape
        fft = torch.fft.fftn(x, dim=(-3, -2, -1))
        ...
        dist = torch.sqrt(xx**2 + yy**2 + zz**2)
        mask = (dist <= radius_ratio).float()  # low-pass mask
    ...
    def forward(self, x_in):
        ...
        # print("hidden_states_out.shape:", hidden_states_out.shape)
        print("this is orgunetr/swin_unetr.py")
        x_in = x_in.float()
        enc0 = self.encoder1(x_in)
        enc0 = self.freq_attn1(enc0)
        print("enc0_shape:",enc0.shape)
        enc1 = self.encoder2(hidden_states_out[0])
        enc1 = self.freq_attn2(enc1)
        print("enc1_shape:",enc1.shape)
        enc2 = self.encoder3(hidden_states_out[1])
        enc2 = self.freq_attn3(enc2)
        print("enc2_shape:",enc2.shape)
        enc3 = self.encoder4(hidden_states_out[2])
        enc3 = self.freq_attn4(enc3)
        print("enc3_shape:",enc3.shape)
        dec4 = self.encoder10(hidden_states_out[4])
        print("dec4_shape:",dec4.shape)
        dec3 = self.decoder5(dec4, hidden_states_out[3])
        print("dec3_shape:",dec3.shape)
        dec2 = self.decoder4(dec3, enc3)
        print("dec2_shape:",dec2.shape)
        dec1 = self.decoder3(dec2, enc2)
        print("dec1_shape:",dec2.shape)
        dec0 = self.decoder2(dec1, enc1)
        print("dec0_shape:",dec0.shape)
        enc1_out = self.enc1_down(enc1)  # [B, out_channels, D, H, W]
        print("enc1_out_shape:",enc1_out.shape)
        enc3_out = self.enc3_up(enc3)  # [B, out_channels, D, H, W]
        print(" enc3_out_shape:",enc3_out.shape)
        enc1_out_hp = self.high_pass_filter(enc1_out, radius_ratio=0.4)  # Adjust 0.1~0.3 for aggressiveness
        enc1_out_fused = enc1_out + 1.5 * enc1_out_hp
        enc3_out_lp = self.low_pass_filter(enc3_out, radius_ratio=0.25)
        enc3_out_fused = enc3_out + 1.5 * enc3_out_lp

        out = self.decoder1(dec0, enc0)
        print("out_shape:",out.shape)
        tumor_out = torch.cat([enc1_out_fused, out], dim=1)
        print("tumor_out_shape:",tumor_out.shape)
        organ_out = torch.cat([enc3_out_fused,out],dim=1)
        print("organ_out_shape:",organ_out.shape)        
       
        # print("logits_tumor_shape:",logits_tumor.shape)
        logits_organ = self.organ_head(organ_out)
        # target_channel = logits_organ[:, 1]  # shape: (B, H, W, D)
